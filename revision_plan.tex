...
While the $rate_{nic}$ can be determined via system call and the datacenter network topology is relatively fixed, we assume the $rtt$ is known a priori.
...
Thanks for the reviewer’s comments. 
Unfortunately, we have not taken into account the fairness issue in this scenario, but it can be got by a simple design.
...
Thanks for the reviewer’s comments. 
So sorry, we made a typing error in the drawing causing the issue.
