...
We aim to explore a new transport that can achieve comparable performance to proactive transports while being readily deployable.
To this point, we start with DCTCP and use LCP to gracefully utilize the available bandwidth. Moreover, we complement its design with a buffer-aware flow scheduling to optimize small flow’s performance.
We then show that PPT can achieve performance comparable to proactive transports by overall average, small flow average/ tail, and large flow average FCT.
We proved the readily deployable nature of PPT by implementing PPT based on Linux-kernel and low CPU overhead.
The above performance metrics can back up our design goals.
...
We will also add a discussion of buffer-aware flow scheduling in Appendix Section 3.1.
...
Thanks for the reviewer’s comments.
We will complete the missing details about the experimental setup and parameters in the form of tables or footnotes.
...
Thanks for the reviewer’s comments. 
Since $BDP = rtt*rate_{nic}$.
While the $rate_{nic}$ can be read via system call and the datacenter network topology is relatively fixed, we assume the $rtt$ is known a priori.
So we can calculate the BDP in advance.
}
...
Thanks for the reviewer’s comments. 
Actually, it is very rarely in our experiments that $\alpha$ is greater than 0.5, as it requires the number of ECN marked packets to be more than half of the number of all packets over many sequential RTTs.
While $\alpha$ greater than 0.5 represents the link experienced severe congestion, there is no spare bandwidth for LCP.
...
Thanks for the reviewer’s comments. 
It is true that we have not taken into account the fairness issue in this scenario, but it can be solved by a simple design.
We will discuss this in Appendix A.
}
...
Thanks for the reviewer’s comments. 
We will add a new simulation experiment with topology consisting of 144 servers, 9 leaf switches, and 4 spine switches, with the host and core links operated at 100 and 400Gbps, respectively.
...
Thanks for the reviewer’s comments. 
We measure the kernel space CPU overhead of PPT and DCTCP in our testbed.
...
We will correct the error and double-check the rest of data.
...
Thanks for the reviewer's comments. 
We will add intuitive reasons to Section 6.2.
...
Thanks for the reviewer's comments.
Homa's poor performance in the testbed is due to the inefficient implementation of the Homa-Linux network stack.
We have explained this in the \emph{remark} paragraph in Section 6.1.1.
...
Thanks for the reviewer’s comments. 
In our paper, we have constructed the variant of PPT that assigns all packets with two priority queues.
We plot the results in Figure 19.
PPT still achieves the optimal overall average FCT among NDP, Homa, Aeolus, DCTCP, and RC3.
}
...
Thanks for the reviewer’s comments. 
Our current topology is commonly used in academia.
Simulation experiments containing 2k nodes require significant time consumption, which we will implement in future work.
