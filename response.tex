...
\begin{document}
\title{Response for SIGCOMM'24 Paper \#293}
...
\noindent Thank you for your email.
We have carefully went through the reviews and addressed them in the responses below. Please let us know if you have any feedback. Thank you for your time and help to improve the quality of our paper!\\
...
\textcolor{blue}{So, in summary, we are not simply dividing the traffic into four class of flows and coupling flows with different priorities to utilize the spare bandwidth. Instead, we divide each individual flow into two parts: the first part runs with high priorities while the second part runs with low priorities to utilize the spare bandwidth in the network. Moreover, the priority queuing is a incremental design to further optimize the average FCT of small flows. 
}
...
\noindent\textcolor{red}{\textbf{Specific modification: }
According to the response above, we have added one paragraph to address the reviewer's concern on the necessity of flow scheduling on dual-loop rate control. Please see the blue paragraph on Column 1, Page 5 in our revised paper.
}
...
Finally, PCC Proteus requires application to specify its priorities, while PPT gradually demotes a flow's priority as more bytes sent.}

\noindent\textcolor{red}{\textbf{Specific modification: }
We have added a footnote 2 on Column 2, Page 4 in our revised paper to discuss the difference between PPT and PCC Proteus.
}
...
\noindent\textcolor{red}{\textbf{Specific modification: }
According to the above responses, we have added one paragraph to show more insights about the spare bandwidth in queue build-up phase. Please see the blue paragraph on Column 1, Page 4 in our revised paper.
}
...
First, we have already implemented a delay-based transport conceptually equivalent to Swift in the ns3 simulator and showed that PPT's design can improve the performance of the original delay-based transport. Please see Fig. 14 for more details.
On the other hand, we have also discussed about the compatibility of PPT with other transports. 
}

\noindent\textcolor{red}{\textbf{Specific modification: }
We have moved the experiment of how PPT works with delay-based transport to the main body to make the paper self-contained.
Please see the blue text on Column 1, Page 12 in our revised paper.
}
...
\noindent\textcolor{red}{\textbf{Specific modification: }
We have added a new Table 3 to summarize the parameter settings. Please see Table 3 on Column 1, Page 14.
}
...
\noindent\textcolor{red}{\textbf{Specific modification: }
We have added more text to clarify this point. Please see the blue text on Column 1, Page 5 in our revised paper.
}
...
\noindent\textcolor{red}{\textbf{Specific modification: }
	We have added a footnote 3 on Column 2, Page 4 in our revised paper to discuss the difference between PPT and PCC Proteus.
}
...
\noindent\textcolor{red}{\textbf{Specific modification: }
As previous version has already included detailed analyses about the performance benefits of PPT, we will not add additional explanation for this point in our revised paper.
}
...
This is a very extreme example and would not behave as the reviewer described.
Let's first rephrase the example the reviewer mentioned. Saying that there is a flow 1 that is currently using the full link bandwidth. Then, another 10 flows (flow 2,$\cdots$,11) arrive.
In this case, the opportunistic packets for flow 1 will not be sent, because queue build-up forms and more ECN-marked packets will be exhibited by flow 1, thus leading the value of its $\alpha$ keep increasing. Whereas, PPT only initialize an LCP loop and send opportunistic packet for a flow when its $\alpha$ takes minimum value. So, PPT will not open an LCP loop for flow 1 in this situation.}
...
\noindent\textcolor{red}{\textbf{Specific modification: }
According to the above responses, we will not add new text in our revised paper to discuss this extreme example.
}
...
\noindent\textcolor{red}{\textbf{Specific modification: }
Based on the above analysis, we will not add further explanation in the revised paper.
}
...
\noindent\textcolor{red}{\textbf{Specific modification: }
According to the responses above, we will not add new text in our revised paper to explain this point.
}
...
But this may not matter too much.
This is because in PPT's design, only the congestion windows after the congestion-avoidance phase are considered for computing $W_{max}$.
}

\noindent\textcolor{red}{\textbf{Specific modification: }
We have added a footnote 3 on Column 1, Page 6 in our revised paper to discuss this point.
}
...
\noindent\textcolor{red}{\textbf{Specific modification: }
We have added a new simulation with 100/400G topology to demonstrate this point. Please see the blue text on Column 1, Page 18 in the revised paper.
}
...
Please see Fig. 21 in Appendix D.2 for more details.
}

\noindent\textcolor{red}{\textbf{Specific modification: }
According to the above response, we will not add new text in our revised paper to address this point.
}
...
\noindent\textcolor{red}{\textbf{Specific modification: }
We have corrected this typo. Please see Fig. 13(a) on page 12 in our revised paper.
}
...
\noindent\textcolor{red}{\textbf{Specific modification: }
We have added more intuitive reasons about this point. Please see the blue text on Column 1, Page 15 in our revised paper.
}
...
\noindent\textcolor{red}{\textbf{Specific modification: }
As we have already explained this motivation in section 2.3, we will not add further explanation in our revised paper.
}
...
Thanks for the comments. The variant actually doesn't include Swift's component that handles endpoint congestion. 
} 

\noindent\textcolor{red}{\textbf{Specific modification: }
We have added a new footnote 7 on Column 1, Page 12 in our revised paper to clarify the exact algorithm the reviewer mentioned.
}
...
\noindent\textcolor{red}{\textbf{Specific modification: }
According to the response above, we will not add further explanation in our revised paper.
}
...
Besides, we have also already compared PPT with HPCC. Please see Fig. 16.
}

\noindent\textcolor{red}{\textbf{Specific modification: }
According to the above response, we will not add further explanation in the revised paper.
}
...
Simulation with 2K-node scale takes significant time, which may miss the camera-ready deadline.
%We will add a large-scale simulation experiment with 2k nodes to the appendix.
} 

\noindent\textcolor{red}{\textbf{Specific modification: }
We plan to do it in the future.
}
...
\noindent\textcolor{red}{\textbf{Specific modification: }
We have added a footnote 3 on Column 1, Page 6 in our revised paper to discuss the fairness issue.
}
